...
> **Status (2026-09-22):** the two content-agnostic engine pieces shipped as M28.3 (dormant, fixture-tested per the M21.6 precedent): **role styling** (`--role-*` tokens + `.role-topic/.role-receiver/.role-description` + `renderRolePattern()`) and **`acceptedOrders` grading** (`isAcceptedTileOrder()` in `build`, plus its build-time validator). The two content-coupled pieces are deferred so they land with the content that exercises them: **French content rendering → M29** (it needs the UI-language state M29 introduces; building it before French can be switched on would be untestable), and **Smart Nudge → the A1.2 content batch** (its triggers are A1.2+ grammar milestones, and its payloads are A1.2 objects).
