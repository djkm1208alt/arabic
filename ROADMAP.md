...
`main` @ `d9b6596`. **M6–M19 merged and live**, plus **M14.1 / M14.5 / M15.5 / M19.5 / M20.5 / M21.5** (sub-milestones layered in via PR #14). M20 (complete A1 content) and M21 (scale-out) — the actual content-authoring milestones, as opposed to their tooling/insights ".5" siblings — have not started. Next: **M20**, or **M22** (student accounts) once a backend is chosen — M22/M23 are also what unblock M26 (Community features).
...
| **M19** | Review / retention | `InteractionEvent` + per-object `ReviewState` (backfilled from existing quizzes); an SM-2-lite scheduler as a pluggable module; a mistake log; review queues. The crude `mastered` toggle becomes `learned → practised → weak → forgotten → reviewed → retained`. | ✅ merged (PR #16, fix in #17) — `progress.interactionLog` (capped 400) + `progress.reviewState`, SM-2-lite `scheduler` module (pure, isolated), `foldReviewState()` incremental fold via `reviewCursor`, `reviewQueue()` (due/mistakes/weak/stale), `#view-review` runner + Practice "Time to review" card. | M18 |
