...
`main` @ `958946c`. **M6–M19 + the .5 batch (M14.1 / M14.5 / M15.5 / M19.5 / M20.5 / M21.5) + M20 Phase A + Phase B batches 1–4 merged and live.** `wordlists/a1.json`'s A1 vocabulary checklist is fully closed (317 lexemes, including this PR's B3 residual cleanup). In progress: A1 writing (batch 5).
...
| **M19.5** | Local learner state (IndexedDB) | An unbounded, queryable `interactionEvents` companion store in IndexedDB alongside (not replacing) M19's capped 400-event `interactionLog` — `progress` stays synchronous/localStorage-backed. Scope narrowed once grounded in the real merged M19, which already solved the unbounded-blob problem pragmatically; see the scope doc §1. `safeIdb*` wrappers, versioned backfill, `getFullInteractionHistory()` for M21.5. | 🧪 implemented, pending review — see [m19.5_local_learner_state_scope.md](m19.5_local_learner_state_scope.md), [content/m19.5-local-learner-state-review.md](content/m19.5-local-learner-state-review.md) — `tools/qa-harness.js` 31/31, `tools/a11y-audit.js` clean | M19 |
| **M20** | Content pipeline + complete A1 | `tools/build-content.js` grows a schema + **linguistic linter** (ḥarakāt coverage, transliteration↔Arabic consistency, register, level fit); migrate all existing content into it; then author a **complete, heavily-QA'd A1 across all 8 strands** as the reference level. Internal gate: pipeline proven → then author A1. | ✅ Phase A (PR #18). ✅ B1 grammar (#21). ✅ B2 core vocab (#22). ✅ B3 rest of A1 vocab (#23) — 94 lexemes closing most of `wordlists/a1.json`: school, work, places, shopping, travel, directions, body/health, weather, question words, conjunctions, subject pronouns. ✅ B3 residual cleanup — 7 more lexemes (young/old-person/old-thing, park, work/job, above/on, to-do/to-make) closing the gaps a plain English-gloss diff against the wordlist missed the first time; wired into the existing generated lessons, no new lesson nodes. **`wordlists/a1.json` checklist now fully closed — 317 lexemes.** ✅ B4 (A1 texts, #25) — 20 fully-vowelled A1 texts (16 graded sentences + 4 mini-dialogues), built only from the merged A1 vocab + grammar, `source:"m20"`; 3 data-authored lessons on `a1-u1` / `a1-u4`. Verbless throughout (A1 has no verb-sentence grammar — narrative reading is post-A2). Then A1 writing (B5), wire `a1-u5` (B6). | M15–M19 |
