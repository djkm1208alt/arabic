...
`main` @ `4f9ecd9`. **M6–M19 + the .5 batch (M14.1 / M14.5 / M15.5 / M19.5 / M20.5 / M21.5) + M20 Phase A + Phase B batches 1–3 merged and live.** `wordlists/a1.json`'s A1 vocabulary checklist is now fully closed (317 lexemes). In progress: this PR (B3 residual cleanup) closes the last of it; next up is A1 texts (batch 4).
...
| **M20** | Content pipeline + complete A1 | `tools/build-content.js` grows a schema + **linguistic linter** (ḥarakāt coverage, transliteration↔Arabic consistency, register, level fit); migrate all existing content into it; then author a **complete, heavily-QA'd A1 across all 8 strands** as the reference level. Internal gate: pipeline proven → then author A1. | ✅ Phase A merged (PR #18). ✅ B1 grammar (PR #21) — 7 `gr:` points + 21 sentences + 2 cluster lessons. ✅ B2 core vocab (PR #22) — 54 lexemes (family / food / home / time / verbs + function words). ✅ B3 (rest of A1 vocab, PR #23) — 94 lexemes closing most of `wordlists/a1.json`: school, work, places, shopping, travel, directions, body/health, weather, question words, conjunctions, subject pronouns; 6 new Word Bank topics; 10 generated lessons; `a1-u4` now available. ✅ B3 residual cleanup — 7 more lexemes (young/old-person/old-thing, park, work/job, above/on, to-do/to-make) closing the gaps a plain English-gloss diff against the wordlist missed the first time; all wired into existing generated lessons, no new lesson nodes. `wordlists/a1.json` checklist now fully closed — 317 lexemes total. Then A1 texts (B4), A1 writing (B5), wire `a1-u5` (B6). | M15–M19 |
