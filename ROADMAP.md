...
| **M20** | Content pipeline + complete A1 | `tools/build-content.js` grows a schema + **linguistic linter** (ḥarakāt coverage, transliteration↔Arabic consistency, register, level fit); migrate all existing content into it; then author a **complete, heavily-QA'd A1 across all 8 strands** as the reference level. Internal gate: pipeline proven → then author A1. | ✅ Phase A merged (PR #18) — linter + A1 word-list spec + lessons-as-data. 🔨 Phase B in progress: **batch 1 (A1 grammar)** — 7 new `gr:` points (number, definiteness, nominal sentence, demonstratives, possessives, prepositions, negation) + 21 cited example sentences + 2 data-authored cluster lessons. Then A1 vocab, then A1 texts. | M15–M19 |
