...
| **M14.1** | UI baseline (mobile-first, RTL, accessible) | Audit-and-harden pass on the *existing* UI — WCAG 2.5.5 touch targets, measured contrast, explicit `dir` alongside `lang`, focus-visible states, a documented breakpoint set — before M15/M16 build more UI on top of it. Not a redesign; M24 owns that. | 🔜 proposed — see [m14.1_ui_baseline_scope.md](m14.1_ui_baseline_scope.md) | M14 |
| **M14.5** | Content validation framework | `tools/build-content.js --lint` (structural: dangling ids, duplicates, ḥarakāt coverage) + `tools/qa-harness.js` (Playwright regression battery), formalizing the manual QA process used for every milestone so far. First npm devDependency. | 🔜 proposed — see [m14.5_content_validation_scope.md](m14.5_content_validation_scope.md) | M14 |
