...
| **M20** | Content pipeline + complete A1 | `tools/build-content.js` grows a schema + **linguistic linter** (ḥarakāt coverage, transliteration↔Arabic consistency, register, level fit); migrate all existing content into it; then author a **complete, heavily-QA'd A1 across all 8 strands** as the reference level. Internal gate: pipeline proven → then author A1. | ✅ Phase A merged (PR #18) — linter + A1 word-list spec + lessons-as-data. ✅ Phase B batch 1 merged (PR #21) — 7 A1 `gr:` points + 21 example sentences + 2 cluster lessons. 🔨 batch 2 (A1 core vocab) — 54 new lexemes across family / food / home / time / daily verbs + the batch-1 function words (هَذَا فِي عَلَى مِنْ إِلَى مَعَ لَيْسَ), wired via 6 generated `a1-u3` lessons. Then rest of A1 vocab, then A1 texts. | M15–M19 |
