...
**Status:** implemented. Shipped exactly as scoped below, with one adjustment made during
implementation: the lesson title originally planned packed all three Arabic terms in cited form,
pushing it over `qa-harness.js`'s 50% "dedicated Arabic content" lang/dir-coverage threshold (the
same class of issue M21.7 hit) — retitled to cite only the flagship pair (إِنَّ / أَنَّ), matching
this project's other titles' ratio. No other change from what's scoped here.
`npm run content:check` clean at 597 objects (406 lexemes, 21 grammar points, 109 texts, 38 units,
65 lessons); `npm run qa` 80/80; `node tools/a11y-audit.js` clean.
